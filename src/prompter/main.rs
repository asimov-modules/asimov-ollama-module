...
#[cfg(not(feature = "std"))]
compile_error!("asimov-ollama-prompter requires the 'std' feature");
...
pub fn main() -> Result<SysexitsError> {
    ...
    let Ok(manifest) = asimov_module::ModuleManifest::read_manifest("ollama")
    ...
    let response =
        asimov_ollama_module::generate(&input, &options).context("failed to generate response")?;
